...
def GET(website,methodselect='G'):
		try:
			try:
				grey = Style.DIM+Fore.WHITE
				...
				paraname = []
				paravalue = []
				wordlist = raw_input("[?] Enter location of Wordlist (Press Enter to use default xsspayloadlist.txt)\n[?] > ")
				if len(wordlist) == 0:
					wordlist = 'xsspayloadlist.txt'
...
def POST(website,methodselect='P'):
	try:
		try:
			try:
...
def xssscanmain(website):
	try:
		methodselect = raw_input("[?] Select method: [G]ET or [P]OST (G/P): ").lower()
		if methodselect == 'g':
			global GET
			GET(website,methodselect)
			return
		elif methodselect == 'p':
			global POST
			POST(website,methodselect)
			return
		else:
